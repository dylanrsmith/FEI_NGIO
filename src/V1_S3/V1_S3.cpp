...
void writeSlotBoard(byte slotNumber, byte command, byte slot_type) {
  ...
  byte colorData[] = {'$', command, slot_type, 0x01, '\n', 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF};
  uint16_t checkSum = crc.checksumCalculator(colorData, 5);
  colorData[5] = checkSum;
  colorData[6] = checkSum >> 8;

  Serial.print("TX: ");
  for (int i = 0; i < 11; i++) {   // 8 instead of 11? both work
  ...
  Serial.print(" [C3 0x");
  uint16_t checksumFromC3 = misoData[5] + (misoData[6] << 8);
